...
csvFILE='./coords_lm.csv'#Excel file including annotated 3d coordinates for 19 anatomical landmarks
datasetDIR='/----REPLACE----/Dataset/' #Path to dicom dataset
outputDIR = './'
configDIR = '../configFiles/3DCnn/' #Path to Deepmedic configuration files
cfgDIR = '../../../CTdataset/'
...
def extractROI(image):
    ...
    thresholder.SetUpperThreshold(-1025) #Air
...
def preprocessImage(input_image,itcounter,lottery,land3D,landmarkList):
    ...
    #################### Recalculate resampled image size and spacing including padding####################
    if(verbose):
...
################### File initialization ####################
    ...
    ntrain=int(nSubjects*70/100)
    nval=int(nSubjects*15/100)
    ntest=int(nSubjects*15/100)
    if(ntrain+nval+ntest<nSubjects):
        ntrain=nSubjects-(ntest+nval)
    ...
    #     optionalvolumes=y
        ...
        if(idealvolume):
            ...
            if(has3Dlandmark):
                ...
                result,output_size=preprocessImage(input_image,counter,lottery,land3D,landmarkList)
                #programPause = raw_input("Press the <ENTER> key to continue...")
